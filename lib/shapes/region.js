...
function CanvasRegion( queue, ui, region ) {
    ...
    queue.subscribe( 'region.moved', function( data ) {
      if ( region.id === data.id &&
          ( shape.getX() != region.getX() || shape.getY() != region.getY() ) ) {
        __moveTo( region.getX(), region.getY() );
      }
    });

    queue.subscribe( 'region.resized', function( data ) {
      if ( region.id === data.id &&
          ( shape.getWidth() != region.getWidth() || shape.getHeight() != region.getHeight() ) ) {
        __resizeTo( region.getWidth(), region.getHeight() );
      }
    });

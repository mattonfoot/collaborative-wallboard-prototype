...
function CanvasRegion( queue, region ) {
    ...
    shape
      .on('mousedown touchstart', function() {
        ...
        queue.publish( 'region.activate', region );
      })
      .on('mouseup touchend', function() {
        __displayInactiveState();

        queue.publish( 'region.deactivate', region );
      })
      .on('dragend', function() {
        queue.publish( 'region.move', { id: region.getId(), x: shape.getX(), y: shape.getY() } );
      })
      .on('dblclick dbltap', function( e ) {
        e.cancelBubble = true;
        shape.getStage().preventEvents = true;

        queue.publish( 'region.edit', region.getId() );
      });
    ...
    handle
      ...
      .on('dragend', function( evt ) {
        ...
        queue.publish( 'region.resize', { id: region.getId(), width: background.getWidth(), height: background.getHeight() } );
      });

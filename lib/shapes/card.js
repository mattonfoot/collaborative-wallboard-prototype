...
function CanvasCard( queue, location, pocket ) {
    ...
    queue.subscribe( 'cardlocation.updated', function( data ) {
    ...
    queue.subscribe( 'pocket.updated', function( data ) {
      if ( pocket.id === data.id ) {
          __UpdateDisplay( data );
      }
    });

    queue.subscribe( 'pocket.transformed', function( data ) {
      if ( pocket.id === data.id ) {
          __UpdateDisplay( data );
      }
    });
    ...
    shape.on('mousedown touchstart', function() {
      ...
      queue.publish( 'cardlocation.activate', location );
    });

    shape.on('mouseup touchend', function() {
      __displayInactiveState();

      queue.publish( 'cardlocation.deactivate', location );
    });

    shape.on('dragend', function() {
      queue.publish( 'cardlocation.move', { id: location.getId(), x: shape.getX(), y: shape.getY() } );
    });
    ...
    shape.on('dblclick dbltap', function( e ) {
      ...
      queue.publish( 'pocket.edit', pocket.getId() );
    });

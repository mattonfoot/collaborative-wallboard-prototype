...
(function ( root, factory ) {
  if ( typeof module === "object" && module.exports ) {
    ...
    module.exports = function( postalFed, io ) {
      return factory( require( 'underscore' ), io, postalFed, require( 'machina' ), require( 'riveter' ));
    }
  ...
  var SocketIoClient = machina.Fsm.extend({
  	...
  	initialize: function() {
  		...
  		self.initialState = (self.target.disconnected ? "disconnected" : "connected" )
  	},
  ...
  var plugin = postal.fedx.transports[ SOCKET_IO ] = {
  	...
  	getTargets : _isServer ?
  	             ...
  	             function() {
  		             return _.reduce(io.managers, function(memo, socket ) {
  			             _.each(socket.nsps, function(ns) {
  				             memo.push(ns);
  			             });
  			             return memo;
  		             }, []);

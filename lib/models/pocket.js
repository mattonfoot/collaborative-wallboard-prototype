...
function Pocket( data ) {
    ...
    this.queue.publish( 'pocket.created', this );

    this.queue.subscribe( 'pocket.updated', Pocket.prototype.updated.bind( this ) );
//  this.queue.subscribe( 'cardlocation.created', Pocket.prototype.cardLocationAdded.bind( this ) );
//  this.queue.subscribe( 'pocket.regionentered', Pocket.prototype.addRegion.bind( this ) );
//  this.queue.subscribe( 'pocket.regionexited', Pocket.prototype.removeRegion.bind( this ) );
}
...
Pocket.prototype.update = function( data ) {
  var update = {
    id: this.id,
    title: data.title
  }
  ...
  this.queue.publish( 'pocket.updated', update );
};

Pocket.prototype.updated = function( data ) {
  if ( data.id === this.id ) {
...
Pocket.constructor = function( data, queue ) {
    if ( data instanceof Pocket ) {
        return data;
    }

    return new Pocket( data, queue );
};

Pocket.factory = function( queue ) {
  Pocket.prototype.queue = queue;

  return function( data ) {
    return Pocket.constructor( data );
  };
};

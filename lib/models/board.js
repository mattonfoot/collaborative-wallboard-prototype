...
function Board( data ) {
    ...
    this.queue.publish( 'board.created', this );

    this.queue.subscribe( 'board.updated', Board.prototype.updated.bind( this ) );
//  this.queue.subscribe( 'region.created', Board.prototype.addRegion.bind( this ) );
//  this.queue.subscribe( 'cardlocation.created', Board.prototype.addCard.bind( this ) );
//  this.queue.subscribe( 'transform.created', Board.prototype.addTransform.bind( this ) );
}
...
Board.prototype.update = function( data ) {
  ...
  this.queue.publish( 'board.updated', update );
};

Board.prototype.updated = function( data ) {
  if ( data.id === this.id ) {
    this.name = data.name;
  }
};
...
Board.constructor = function( data, queue ) {
    if ( data instanceof Board ) {
        return data;
    }

    return new Board( data, queue );
};

Board.factory = function( queue ) {
  Board.prototype.queue = queue;

  return function( data ) {
    return Board.constructor( data );
  };
};

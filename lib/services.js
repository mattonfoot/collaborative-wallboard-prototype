...
function Services( interface, commands, queries, queue ) {
  var services = this;
  var interface = this._interface = interface;
  var commands = this._commands = commands;
  var queries = this._queries = queries;
  var queue = this._queue = queue;

  // setup events to trigger services

  var listeners2 = [ 'new', 'create', 'edit', 'update', 'select', 'display', 'unlink', 'move', 'resize', 'transform' ];

  models.forEach(function( model ) {
      setUpEventListeners( model );
  });

  function setUpEventListeners( type ) {
      listeners2.forEach(function( task ) {
          queue.subscribe( type.toLowerCase() + '.' + task, function( ev ) {
              if (!services[ task + type ]) return;
              ...
              if ( promise && promise.catch ) {
                promise
                  .catch(function( error ) {
                    queue.publish( type.toLowerCase() + '.' + task + '.fail', { message: error.message, stack: error.stack } );
                  });
              }
          })
          .catch(function( error ) {
...
Services.prototype.createBoard = function( data ) {
  ...
  return commands.createBoard( data )
    ...
    .then(function() {
      queue.publish( 'board.created', board );
...
Services.prototype.addPocketToBoards = function( boards, pocket ) {
  ...
  boards.forEach(function( board ) {
    promise = promise
      ...
      .catch(function( error ) {
        queue.publish( 'cardlocation.created.fail', { message: error.message, stack: error.stack } );
      })
      .then(function( location ) {
        queue.publish( 'cardlocation.created', location );
...
Services.prototype.moveRegion = function( info ) {
  ...
  return queries.getRegion( info.id )
    ...
    .then(function( region ) {
      queue.publish( 'region.moved', info ); // --> region:updated
    });
};
...
Services.prototype.resizeRegion = function( info ) {
  ...
  return queries.getRegion( info.id )
    ...
    .then(function( region ) {
      queue.publish( 'region.resized', info ); // --> region:updated
    });
};
...
Services.prototype.updateRegion = function( data ) {
  ...
  return queries.getRegion( data.id )
    ...
    .then(function( region ) {
      queue.publish( 'region.updated', data ); // --> region:updated
    });
};
...
Services.prototype.createWall = function( data ) {
  ...
  return commands.createWall( data )
    ...
    .then(function( wall ) {
      queue.publish( 'wall.created', wall );

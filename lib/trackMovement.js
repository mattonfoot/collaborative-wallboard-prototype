...
MovementTracker.prototype.trackCardMovement = function( location ) {
  var tracker = this
    ...
    tracker._queries.getPocket( location.getPocket() )
        ...
        .then(function( regions ) {
            ...
            patch.out.forEach(function( region ) {
              tracker._queue.emit( 'pocket:regionexit', { pocket: pocket, region: region } );
            });

            patch.in.forEach(function( region ) {
              tracker._queue.emit( 'pocket:regionenter', { pocket: pocket, region: region } );
            });
        });
};
...
MovementTracker.prototype.trackRegionMovement = function( region ) {
  ...
  tracker._queries.getBoard( region.getBoard() )
    .then(function( board ) {

...
function UI( queue, $element, options, $ ) {
    ...
    var lock = this.lock = new Auth0Lock( 'X0n9ZaXJrJgeP9V4KAI7LXsiMsn6jN4G', 'vuu-se.auth0.com' );
    ...
    var clicks = [ 'new', 'edit', 'select', 'display', 'unlink', 'auth' ];
...
UI.prototype.displayLogin = function( data ) {
  var ui = this;

  ui.lock.show(function( err, profile, token ) {
    if ( err ) {
      // Error callback

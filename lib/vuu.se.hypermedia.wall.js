...
function init( app, io ) {
  ...
  return {
      ...
      create: function( data ) {
          ...
          return db.create( 'wall', obj )
              .then(  created,
                      function( err ) {
                          io.sockets.emit( 'wall:createfail', {
                              data: data,
                              err: err
                          });
                      }
      ...
      update: function( data ) {
          ...
          return db.update( 'wall', wallid, data )
              .then(  updated,
                      function( err ) {
                          io.sockets.emit( 'wall:updatefail', {
                              data: data,
                              err: err
                          });
                      }
      ...
      delete: function( id ) {
          return db.delete( 'wall', id )
              .then(  deleted,
                      function( err ) {
                          io.sockets.emit( 'wall:deletefail', {
                              id: id,
                              err: err
                          });
                      }
  ...
  function created( wall ) {
      io.sockets.emit( 'wall:created', wall );
  }

  function updated( wall ) {
      io.sockets.emit( 'wall:updated', wall );
  }

  function deleted( wall ) {
      io.sockets.emit( 'wall:deleted', wall );
  }

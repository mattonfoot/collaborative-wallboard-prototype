...
function getFromEventSource( queries, model, id ) {
  return new Promise(function( resolve, reject ) {
    ...
    queries.db.allDocs({
        ...
        startkey: 'vuu.se_' + model + '_' + id
      })
      .then(function( results ) {
        if ( !results.rows || !results.rows.length ) return resolve( [] );

        var resources = results.rows.map(function( result ) { return result.doc; });

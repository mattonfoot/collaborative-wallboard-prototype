...
function Application( belt, queue, ui, options ) {
    ...
    var commands = this.commands = new Commands( belt, queue ); // memento storage
    var queries = this.queries = new Queries( belt, queue );    // event sourced objects


    var interface = this.interface = new Interface( queue, queries, ui );
    var services = this.services = new Services( queue, queries, commands, interface );

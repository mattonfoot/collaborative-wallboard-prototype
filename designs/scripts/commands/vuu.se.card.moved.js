...
// event <-- card:moved, region:moved, card:updated, card:regionenter, card:regionexit
...
function initialize( app ) {
    ...
    queue.on( app, 'region:moved', function( data ) {
        /* console.log( 'region:moved', data ); */
    });

    queue.on( app, 'card:regionenter', function( data ) {
        var pocket = app.wall.getPocketById( data.card.getPocketId() );

        pocket.set( data.board.key, data.region.value );
    });

    queue.on( app, 'card:regionexit', function( data ) {
        var pocket = app.wall.getPocketById( data.card.getPocketId() );
